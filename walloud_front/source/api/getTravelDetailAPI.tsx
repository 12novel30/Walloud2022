...
const GetTravelDetailAPI = async ({
  userId,
  travelId,
  setEventList,
  setPersonList,
  setPeriod,
  setIsManager,
}: TravelDetailAPIProps) => {
  axios
    .get(`/api/${travelId}/getTravelHomeView`)
    .then((response) => {
      console.log(response.data);
      setEventList(
        response.data.eventList.map((event: object) => ({
          ...event,
          isDetail: false,
      ...
      setPersonList(
        response.data.personList.map((person: object) => ({
          ...person,
          detail: { isView: false, sumGet: 0, sumSend: 0, isSettled: false },
        }))
      );
      setPeriod(response.data.period);
      const Manager: PersonProps = response.data.personList.find(
        (p: PersonProps) => p.role === true
      );
